...
@ItemTypeInfo(value = Types.ENCLOSURE_TYPE_LOCAL_ID, layers = {Types.INFRASTRUCTURE_LAYER})
@ConnectedTo(toClass = TMFabricSwitch.class, type = RelationshipNames.CONTAINS,
        typeName = RelationshipNames.CONTAINS_NAME)
@ConnectedTo(toClass = TMNode.class, type = RelationshipNames.CONTAINS, typeName = RelationshipNames.CONTAINS_NAME)
@ConnectedTo(toClass = TMRack.class,
        relationshipDetails = @LoomAttribute(key = "Rack", supportedOperations = {DefaultOperations.GROUP_BY}),
        type = RelationshipNames.CONTAINS, typeName = RelationshipNames.CONTAINS_NAME)
@ActionDefinition(name = "Clear alerts", type = ActionTypes.Thread, id = "Clear enclosure alerts",
        description = "Clear all alerts in the thread", icon = "icon-delete")
@ActionDefinition(name = "Clear alert", type = ActionTypes.Item, id = "Clear enclosure alert",
        description = "Clear the alert", icon = "icon-delete")
@ActionDefinition(name = "Clear alert", type = ActionTypes.Aggregation, id = "Clear enclosure alert",
        description = "Clear the alert", icon = "icon-delete")
public class TMEnclosure extends DmaBaseItem<TMEnclosure.ItemAttributes> {
    ...
    @LoomAttribute(type = NumericAttribute.class, key = "CPU utilisation", min = "0", max = "100", unit = "%",
            plottable = true, supportedOperations = {DefaultOperations.SORT_BY}, ignoreUpdate = true)
    ...
    public TMEnclosure(final String logicalId, final ItemType itemType) {
    ...
    private HashMap<String, NonCoreMetricAccumulation> accumulateSocMetricsAndAttributes() {
        ...
        Collection<Item> nodes = this.getConnectedItemsWithRelationshipName(
        ...
        for (Item nodeItem : nodes) {
            ...
            for (Item socItem : socs) {
                ...
                if (null != soc.getCore().getCpuStolen()) {
                    metAtt = accumulate(metAtt, Const.METRIC_CPU_STOLEN, soc.getCore().getCpuStolen().doubleValue());
                }
                if (null != soc.getCore().getCpuSystem()) {
                    metAtt = accumulate(metAtt, Const.METRIC_CPU_SYSTEM, soc.getCore().getCpuSystem().doubleValue());
                }
